...
let host_regex = 'https://whimsubs.xyz/r/read/*';
...
bookmarklet([
    ...
    "https://stuk.github.io/jszip/vendor/FileSaver.js",
        ...
        async function start(){
            res = await fetch(window.location.href+'/manifest.json');
            json = await res.json();

            let images = [];
            for (img of json.readingOrder) {
                images.push([img.href.split('/').pop(), img.href]);
            }

            download_name = "[WhimSubs]"+json.metadata.belongsTo.series.name.replace(/ /g,'_')+"_c"+json.metadata.belongsTo.series.position.padStart(2,0)+".zip";

            download_images(download_name, images);
        }
